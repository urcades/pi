...
export function parseKey(data: string): string | undefined {
	const formatParsedKey = (codepoint: number, modifier: number, baseLayoutKey?: number): string | undefined => {
		...
		const isLatinLetter = normalizedCodepoint >= 97 && normalizedCodepoint <= 122;
		const isDigit = normalizedCodepoint >= 48 && normalizedCodepoint <= 57;
