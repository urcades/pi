...
1. Never use destructive commands:
   ...
   - `git clean -fd`
   - `git stash`
2. Never sync directly on `main`.
3. Always use a dedicated sync branch (`sync/upstream-YYYYMMDD`).
4. Keep integration batches small and independently verifiable.
5. Preserve fork invariants:
   - Upstream `packages/agent/src/**` maps to local `packages/coding-agent/src/core/agent-core/**`
   - Root scripts `pi-test.sh` / `test.sh` map to `scripts/pi-test.sh` / `scripts/test.sh`
   - Legacy extension import compatibility remains in `packages/coding-agent/src/core/extensions/loader.ts`
6. The update-check banner is intentionally disabled (`checkForNewVersion` returns `undefined`) — do not reintroduce it when porting upstream changes to `interactive-mode.ts`.

## Required references

- Playbook: `docs/upstream-sync-playbook.md`
- Sync log: `docs/upstream-sync-log.md`
...
Pick one bounded window: release tag range, commit range, or date window.
...
- **Direct apply** — paths map cleanly
...
Log each decision in the sync log.
